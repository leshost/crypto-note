...
У версії 1.3.0 додано багатодокументний інтерфейс та покращено взаємодію з файлами:
* **Вкладки (Tabs):** Тепер ви можете відкривати кілька зашифрованих файлів одночасно в одному вікні, як у браузері.
* **Drag & Drop:** Перетягуйте файли `.cnot` з файлового менеджера прямо у вікно програми для швидкого відкриття.
* **Запам'ятовування шрифту:** Ваш вибір розміру тексту (зум) тепер зберігається автоматично і застосовується при наступному запуску до всіх нових вкладок.
* **Покращене автозбереження:** При закритті програми вона акуратно запитає про збереження кожної зміненої вкладки по черзі.

## Оновлення v1.2.0 🌍

У версії 1.2.0 додано повноцінну підтримку багатомовності:
* **Багатомовний інтерфейс:** Тепер програма повністю перекладена англійською та українською мовами.
* **Перемикання "на льоту":** Змінюйте мову інтерфейсу одним кліком за допомогою нової кнопки на панелі інструментів, без необхідності перезапускати програму.
* **Збереження налаштувань:** Ваш вибір мови зберігається автоматично і буде застосований при наступному запуску.
